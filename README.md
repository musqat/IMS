...
이를 줄이기 위해 외부 스케줄러로 10분마다 헬스 체크를 보내 인스턴스를 깨워둡니다. 무료 인스턴스는 월 750시간 한도가 있어 종일 깨워두면 한도에 근접하므로, 낮 시간대(KST 08:00~19:59)와 자정 결산 배치 구간만 유지합니다.

그 외 시간대의 첫 요청은 기동 대기가 발생합니다. 화면이 비어 보이면 잠시 기다린 뒤 새로고침해 주세요. 슬립 이후에도 데이터는 유지됩니다.
